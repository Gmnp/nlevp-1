...
%   norm(A_j, inf) = 10^{2j}. The default values are n=10 and k=2. The
%   matrices are returned in a cell array of length k+1 COEFFS={EYE(n),
%   A_1, ..., A_k}. FUN is a function handle to evaluate the functions
%   lambda, exp(-lambda*tau_1), ..., exp(-lambda*tau_k) and their
%   derivatives. F is the function handle lambda*EYE(n) +
%   \sum_{j=1}^{k}exp(-lambda*tau_j)A_j.
%   This problem has the properties nep, real, scalable
%   parameter-dependent. 
